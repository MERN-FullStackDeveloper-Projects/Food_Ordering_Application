...
import { removeFromCartAction, updateQuantityAction } from "../../slices/cartSlice";
function Cart() {
...
{items.map(item =>{
  ...
  return <div className="item-container" style={{
    justifyContent: 'space-between'}}>
    <img src={imageUrl} className='thumbnail'/>
    <div className="title">{item.title}</div>
    <div className=" description">{item.description}</div>
  <div className="item-container">
    <button onClick={() => onIncrement(item)} className="btn btn-success">+</button>
  <div>{item.quantity}</div>
      <button onClick={() => onDecrement(item)} className="btn btn-success">-</button>
</div>
  </div>
        ...
        <div className="col-3">
          <h4>Summary</h4>
          <div className="title">#items: {items.length}</div>
          <div className="title">Total: Rs.{total}</div>
        </div>
      </div>
      )}
      {items.length == 0 && <h4 className="page-header">there are no items in the Cart</h4> }
